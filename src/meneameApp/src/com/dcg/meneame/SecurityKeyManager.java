...
public class SecurityKeyManager {
	...
	public static String GetSecurityKey( Context context ) {
		...
		SystemValueItem systemValue = SystemValueManager.getSystemValue(context.getContentResolver(), SECURITY_KEY_TIME);
			...
			try {
    			...
    			if (response.getStatusLine().getStatusCode() == HttpStatus.SC_OK) {
					...
					try {
						...
						// We start at the top becasue the base_key is at the end of the document
						for(int i = arrayData.length-1; i >=0 ; i-- )
						{
							if (arrayData[i].startsWith(KEY_PREFIX) ) 
							{
								// We found the base_key line! So parse it!
								securityKey = arrayData[i].substring(KEY_PREFIX.length(), arrayData[i].length() - KEY_SUFFIX.length());
								break;
							}
						}
						
						// Check data
						ApplicationMNM.logCat(TAG, "Security key aquired: " + securityKey);
					} catch (IOException e) {
						ApplicationMNM.warnCat(TAG, "Could not request security key " + URL + ": " + e);
					} catch (Exception e) {
						ApplicationMNM.warnCat(TAG, "Could not request security key " + URL + ": " + e);
						e.printStackTrace();
					} finally {
    			...
    			{
    				ApplicationMNM.warnCat(TAG, "Could not request security key " + URL + " code: " + response.getStatusLine().getStatusCode());
    			}
    		} catch( Exception e ) {
    			ApplicationMNM.warnCat(TAG, "Could not request security key " + URL + ": " + e);
    		} finally {
		...
		if ( !bRequestKey )
		{
			systemValue = SystemValueManager.getSystemValue(context.getContentResolver(), SECURITY_KEY_KEY);
			securityKey = ( systemValue != null && systemValue.mValue != null )?systemValue.mValue:securityKey;
		}
		...
		if ( bRequestKey )
		{
			...
			SystemValueManager.setSystemValue(context.getContentResolver(), SECURITY_KEY_TIME, String.valueOf(now));
			SystemValueManager.setSystemValue(context.getContentResolver(), SECURITY_KEY_KEY, securityKey);
		}
